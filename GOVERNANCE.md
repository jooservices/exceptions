...
`jooservices/exceptions` is an owner-driven project (benevolent-leader style).
...
- Breaking changes are acceptable only in major versions; this line starts at `v4.0.0` as a full rebuild with no backward compatibility

## Quality authority

The repository quality gates (Pint `per`, PHPCS, PHPStan max with strict rules,
PHPMD, docs snippet verify, 100% statement coverage, required Docker CI chain)
